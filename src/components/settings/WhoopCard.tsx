...
export function WhoopCard({ integration }: { integration: WhoopIntegration }) {
  ...
  async function disconnect() {
    if (!confirm("Disconnect Whoop? Your existing data will be kept.")) return;
  ...
  async function syncNow() {
    ...
    try {
      ...
      setSyncMessage(
        `Synced: ${data.sleep ?? 0} sleep, ${data.workouts ?? 0} workouts, ${data.metrics ?? 0} metrics`
      );
      router.refresh();
    } catch (e) {
    ...
    <div className="p-4 bg-zinc-800/50 rounded-xl space-y-3">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-[#FF3D00]/15 flex items-center justify-center">
            <Wifi className="w-4 h-4 text-[#FF3D00]" />
          </div>
          <div>
            <p className="text-sm font-medium text-zinc-50">
              Whoop{integration.provider_user_id ? ` · ID ${integration.provider_user_id}` : ""}
            </p>
      ...
      {syncMessage && (
        <p
          className={`text-xs ${
            syncMessage.startsWith("Synced") ? "text-blue-400" : "text-red-400"
          }`}
        >
          {syncMessage}
        </p>
